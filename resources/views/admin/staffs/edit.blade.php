...
<head>
    ...
    <title>@isset($staff) تعديل مؤطر @else إضافة مؤطر جديد @endisset - برنامج الطالب المتفوق</title>
    ...
    <div class="flex">
        ...
        <aside class="top-0 sticky bg-white shadow-md w-64 h-screen">
            ...
            <div class="p-4">
                <ul class="space-y-2 mt-4">
                    ...
                    <li>
                        <a href="{{ route('admin.levels.index') }}" class="flex items-center space-x-2 space-x-reverse hover:bg-gray-100 p-3 rounded-lg">
                    ...
                    <li>
                        <a href="{{ route('admin.students.index') }}" class="flex items-center space-x-2 space-x-reverse hover:bg-gray-100 p-3 rounded-lg">
                            ...
                            <span>الطلبة</span>
                            <span class="bg-blue-500 px-2 py-1 rounded-full text-white text-xs">{{ App\Models\Student::count() }}</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.staffs.index') }}" class="flex items-center space-x-2 space-x-reverse bg-blue-100 p-3 rounded-lg text-blue-800">
                            ...
                            <span>المؤطرين</span>
                            <span class="bg-blue-500 px-2 py-1 rounded-full text-white text-xs">{{ App\Models\Staff::count() }}</span>
                        </a>
                    </li>
                    <li>
                    ...
                    <li>
                        <a href="{{ route('admin.attendance.index') }}" class="flex items-center space-x-2 space-x-reverse hover:bg-gray-100 p-3 rounded-lg">
                            <i class="fas fa-user-check"></i>
                            <span>الحضور والغياب</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.payments.index') }}" class="flex items-center space-x-2 space-x-reverse hover:bg-gray-100 p-3 rounded-lg">
                            <i class="fas fa-file-invoice-dollar"></i>
                            <span>المدفوعات المالية</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.reports.index') }}" class="flex items-center space-x-2 space-x-reverse hover:bg-gray-100 p-3 rounded-lg">
        ...
        <main class="flex-1 p-8">
            <div class="bg-white shadow-md p-6 rounded-lg max-w-3xl mx-auto">
                ...
                <form method="POST" 
                      action="@isset($staff) {{ route('admin.staffs.update', $staff->id) }} @else {{ route('admin.staffs.store') }} @endisset">
                    @csrf
                    @isset($staff)
                        @method('PUT')
                    @endisset
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" id="birth_date" name="birth_date" 
                                   value="{{ old('birth_date', isset($staff) ? $staff->birth_date->format('Y-m-d') : '') }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                   required>
                            @error('birth_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
